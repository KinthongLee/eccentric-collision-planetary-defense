...
% Show progress in the parfor loop
    ...
    % starts the loop
    tic;
    norm_delta_v_BIP = zeros(sample_size,1);
    norm_delta_v_COG = zeros(sample_size,1);
...
toc;
    ...
    data.different_delta_v_n_relative(p) = data.different_delta_v_n(p) / data.delta_v_n_COG(p);
    data.different_delta_v_h(p) = data.delta_v_h_BIP(p) - data.delta_v_h_COG(p);
    data.different_delta_v_h_relative(p) = data.different_delta_v_h(p) / data.delta_v_h_COG(p);
    ...
    % 保存.mat文件
    save(filename, 'delta_r_min_COG','delta_r_min_BIP','norm_delta_v_COG','norm_delta_v_BIP','angles','delta_v_BIP','delta_v_COG','delta_v_BIP_tnh','delta_v_COG_tnh');
