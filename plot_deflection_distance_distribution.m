...
for i = 1:32
    if i ~= 9 && i ~= 14 % Exclude No.9 & No.14, due to unavailable Lambert Transfer
        ...
        % Deflection distance
        delta_r_min = delta_r_min./1000; % BIP, km
        delta_r_min_COM = delta_r_min_COM./1000; % COG (Center of Mass), km
        % Fix the scale of x-axis to the 0.01% to 99.99% of the total
        % samples(include both BIP & COG strategy)
        minimum = min( prctile(delta_r_min, 0.01),prctile(delta_r_min_COM, 0.01) );
        maximum = max( prctile(delta_r_min, 99.99),prctile(delta_r_min_COM, 99.99) );

        % Plot graph
        % BIP strategy
        gca = figure(1);
        ax1 = subplot(2,1,1);
        histogram(delta_r_min);
